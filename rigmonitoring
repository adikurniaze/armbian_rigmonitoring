...
MINERLOG_FILE="$HOME_DIR/miner.log"
...
echo -e "Pool address : "$(cat $MINERLOG_FILE | grep "Starting on" | awk '{print $5}')"" >> $MSG_FILE
echo -e "Hashrate : "$(tail -1 $MINERLOG_FILE | grep "accept" | awk '{print $7}')" kH/s" >> $MSG_FILE
echo -e "Last share : "$(tail -1 $MINERLOG_FILE | grep "accept" | awk '{print $4}') >> $MSG_FILE
# Send Notif Telegram
TOKEN=5036143682:AAGezuDr1OpJgzIctf1tYWzHNkvNhHK-zJM
CHAT_ID=(1391026878) #ID_TELEGRAM diisi dengan id yang akan menerima notifnya.
